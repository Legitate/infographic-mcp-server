...
PROFILE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), 'chrome-data', 'mcp-profile'))
PROFILE_CACHE_FILE = os.path.join(os.path.dirname(__file__), 'chrome-data', '.chrome_profile_cache.json')
...
async def generate_infographic(url: str, title: str = None):
    ...
    if not auth_state["authenticated"]:
        return "Authentication required. Please visit http://localhost:8000/login to authenticate with Google OAuth."
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-auth", action="store_true", help="Disable Google OAuth requirement")
    ...
    # 2. Start WebSocket Server and HTTP MCP Server
    import threading
    ...
    async def auth(request: Request):
        ...
        return JSONResponse({"message": "Successfully authenticated via Google OAuth. You can now use the MCP server tools.", "user": user})

    def run_mcp_http():
        logger.info("Starting FastMCP HTTP (SSE) Server on port 8000 with Google OAuth...")
        ...
        app.add_middleware(SessionMiddleware, secret_key="infographic-generator-secret-key-12345")
        
        uvicorn.run(app, host="0.0.0.0", port=8000)

    mcp_thread = threading.Thread(target=run_mcp_http, daemon=True)
    mcp_thread.start()

    logger.info("Starting WebSocket server on main thread...")
    try:
